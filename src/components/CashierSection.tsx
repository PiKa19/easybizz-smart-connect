...
const CashierSection = ({
  ...
  const handleUpdateCashier = () => {
    if (editingCashier) {
      setCashiers(prev => prev.map(c => c.id === editingCashier.id ? editingCashier : c));
  ...
  if (selectedCashier) {
    return <PersonnelManagement cashier={selectedCashier} onBack={() => setSelectedCashier(null)} />;
  }
  return <div className="space-y-6">
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="cashier-name" value={newCashier.name} onChange={e => setNewCashier(prev => ({
                ...prev,
                name: e.target.value
              ...
              <div>
                ...
                <Input id="cashier-pin" type="password" maxLength={4} value={newCashier.pinCode} onChange={e => setNewCashier(prev => ({
                ...prev,
      ...
      <Dialog open={isEditCashierOpen} onOpenChange={setIsEditCashierOpen}>
        <DialogContent>
          ...
          {editingCashier && <div className="space-y-4">
              <div>
                <Label htmlFor="edit-cashier-name">Cashier Name</Label>
                <Input id="edit-cashier-name" value={editingCashier.name} onChange={e => setEditingCashier(prev => prev ? {
              ...prev,
              name: e.target.value
            } : null)} />
              </div>
              <div>
                ...
                <Input id="edit-cashier-pin" type="password" maxLength={4} value={editingCashier.pinCode} onChange={e => setEditingCashier(prev => prev ? {
              ...prev,
              pinCode: e.target.value.replace(/\D/g, '')
            } : null)} />
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cashiers.map(cashier => <Card key={cashier.id} className="hover:shadow-lg transition-shadow">
            <CardContent className="p-6">
              <div className="flex flex-col items-center text-center space-y-4">
                <img src={cashier.image} alt={cashier.name} className="w-16 h-16 object-contain" />
                <div>
                ...
                <div className="flex gap-2 w-full">
                  <Button onClick={() => handleManageCashier(cashier)} className="flex-1 bg-pink-500 hover:bg-pink-600 text-white">
                    Manager this cashier
                  </Button>
                </div>
                <Button variant="outline" size="sm" onClick={() => handleEditCashier(cashier)} className="w-full">
        ...
        <Card className="hover:shadow-lg transition-shadow border-dashed border-2 border-gray-300">
          <CardContent className="p-6">
            <div className="flex flex-col items-center justify-center text-center space-y-4 h-full min-h-[200px]">
              ...
              <Button onClick={() => setIsAddCashierOpen(true)} variant="outline" className="w-full">
                Add cashier
              </Button>
            </div>
          </CardContent>
